...
{
	delay(100);
	i2c.WriteRegister(DEVICE_ADDRESS, CTRL_REG1, 0x8F); //multiwii reg1
	i2c.WriteRegister(DEVICE_ADDRESS, CTRL_REG5, 0x02); //multiwii reg5
...
void L3G4200D::Receive()
{
	...
	byte *buf = i2c.ReadRegister(DEVICE_ADDRESS, 0x80|0x28, 6);
	x = (buf[1] << 8) | buf[0];
	y = (buf[3] << 8) | buf[2];
	z = (buf[5] << 8) | buf[4];
}